...
`issue-create`, `issue-verify`, and `issue-fix` consult the `pr` plugin's `pr-sidekick` agent, when installed, for a profile of the repo (templates, test layout, monorepo packages), built fresh on each call, and, in `issue-fix`, for remembered review rules around the fix. That agent is `pr:pr-sidekick` in Claude Code and the `pr-sidekick` subagent in Cursor — install `pr` too to get it; without it, each skill works the same, just without the memory.
