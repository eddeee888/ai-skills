...
- Before writing code, get a `brief` from the `pr:pr-sidekick` agent (`CONVENTIONS.md`), passing the files the chosen option touches and a one-line summary of it — so rules the user's reviewers have already asked for in this repo shape the fix up front. It lives in the `pr` plugin; not installed → skip it.
