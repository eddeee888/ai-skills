...
`pr-sidekick` available (`CONVENTIONS.md`) → get a `profile` of the target repo; it doesn't need to be checked out. It names the bug-report template and its required fields, and flags contribution rules that bind an issue — fetch just that one template's full text. If it couldn't tell which template is the bug report, handle it as below. Not available → look it up inline:
