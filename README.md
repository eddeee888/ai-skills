...
The `pr` plugin also ships an agent, [`pr-sidekick`](pr/agents/pr-sidekick.md): a sidekick with persistent memory of your recurring review themes, which the `pr` skills and `oss:issue-fix` consult when classifying threads, coding, and writing PR descriptions (Claude Code only).
